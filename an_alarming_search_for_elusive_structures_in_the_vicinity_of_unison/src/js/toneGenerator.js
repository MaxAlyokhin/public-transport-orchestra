...
// В 23 строке можем менять порг Гц у фильтра, если выскакивают паразитные частоты
...
export function toneGenerator(toneDuration, toneFrequency) {
  ...
  biquadFilter.Q.setValueAtTime(1, currentTime) // Фильтр на полную
  biquadFilter.connect(gainNode) // Подключаем к ручке громкости

  // Настраиваем ручку громкости
  gainNode.gain.setValueAtTime(0.005, currentTime) // Громкость одного тона должна быть кратна количеству одновременно звучащих тонов

  gainNode.gain.exponentialRampToValueAtTime(0.2, currentTime + 0.025) // Громкость одного тона должна быть кратна количеству одновременно звучащих тонов
